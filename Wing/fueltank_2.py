...
class FuelTank(GeomBase):
    ...
    def trimmed_root_inner(self):
        return TranslatedCurve(curve_in=self.trimmed_root_inner_scaled.curve,
                               displacement=self.trimmed_root_inner_scaled.cog.vector_to(self.trimmed_root.cog))
    ...
    @Part
    def outer_surf(self):
        return RuledSolid(profiles=[self.trimmed_tip_mirrored, self.trimmed_root, self.trimmed_tip],
                          transparency=0.4)

    @Part
    def inner_surf(self):
        return RuledSolid(profiles=[self.trimmed_tip_mirrored_inner, self.trimmed_root_inner, self.trimmed_tip_inner],
                          transparency=0.7)

    @Part
    def fuel_tank(self):
        return SubtractedSolid(shape_in=self.outer_surf, tool=self.inner_surf)
